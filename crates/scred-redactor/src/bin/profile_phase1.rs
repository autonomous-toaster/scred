...
fn main() {
    ...
    println!();
    println!("Detection time: {:.1}%", (avg_detect / avg_combined) * 100.0);
    println!("Redaction time: {:.1}%", (avg_redact / avg_combined) * 100.0);
    ...
    println!("Needed for 125 MB/s target: {:.1}x improvement", 125.0 / combined_throughput);
}
