use scred_redactor::{StreamingConfig, StreamingEvent, StreamingRedactor};
...
fn main() {
    ...
    let per_byte_ns =
        (elapsed.as_nanos() as f64 / (iterations as f64 * test_data.len() as f64)) as f64;
    ...
    println!(
        "Throughput: {:.2} MB/sec",
        (test_data.len() as f64 * iterations as f64) / (elapsed.as_secs_f64() * 1024.0 * 1024.0)
    );
