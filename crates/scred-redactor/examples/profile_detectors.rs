...
fn main() {
    ...
    eprintln!(
        "simple_prefix:  {:6}ms, {} matches, {:.1} MB/s",
        t1.as_millis(),
        r1.matches.len(),
        10.0 / t1.as_secs_f64()
    );
    ...
    eprintln!(
        "validation:     {:6}ms, {} matches, {:.1} MB/s",
        t2.as_millis(),
        r2.matches.len(),
        10.0 / t2.as_secs_f64()
    );
    ...
    eprintln!(
        "uri_patterns:   {:6}ms, {} matches, {:.1} MB/s",
        t5.as_millis(),
        r5.matches.len(),
        10.0 / t5.as_secs_f64()
    );
    ...
    eprintln!(
        "simple_prefix:  {:.1}%",
        t1.as_secs_f64() * 100.0 / total.as_secs_f64()
    );
    eprintln!(
        "validation:     {:.1}%",
        t2.as_secs_f64() * 100.0 / total.as_secs_f64()
    );
    eprintln!(
        "jwt:            {:.1}%",
        t3.as_secs_f64() * 100.0 / total.as_secs_f64()
    );
    eprintln!(
        "ssh_keys:       {:.1}%",
        t4.as_secs_f64() * 100.0 / total.as_secs_f64()
    );
    eprintln!(
        "uri_patterns:   {:.1}%",
        t5.as_secs_f64() * 100.0 / total.as_secs_f64()
    );
