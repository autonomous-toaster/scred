...
fn main() {
    ...
    println!(
        "- Simple prefix: {:.1}% ({:.3}s / {:.3}s)",
        (avg_simple / avg_all) * 100.0,
        avg_simple,
        avg_all
    );
    println!(
        "- Validation:    {:.1}% ({:.3}s / {:.3}s)",
        (avg_validation / avg_all) * 100.0,
        avg_validation,
        avg_all
    );
    println!(
        "- JWT:           {:.1}% ({:.3}s / {:.3}s)",
        (avg_jwt / avg_all) * 100.0,
        avg_jwt,
        avg_all
    );
    println!(
    ...
    println!(
        "Gap: {:.1}x improvement needed ({:.0}%)\n",
        125.0 / tput_all,
        (1.0 - tput_all / 125.0) * 100.0
    );
    ...
    let slowest = components
        .iter()
        .min_by(|a, b| a.1.partial_cmp(&b.1).unwrap())
        .unwrap();

    println!(
        "🔴 SLOWEST COMPONENT: {} ({:.1} MB/s)",
        slowest.0, slowest.1
    );
    ...
    println!("Next Steps:");
    println!(
        "1. Identify if Aho-Corasick is the bottleneck in {} detection",
        slowest.0
    );
