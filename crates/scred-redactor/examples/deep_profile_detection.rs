...
fn main() {
    ...
    println!("detect_all() Total: {:.3}s, {:.1} MB/s\n", 
    ...
    let time_measured = time_simple.as_secs_f64() + time_validation.as_secs_f64() + time_jwt.as_secs_f64();
    let time_other = elapsed_all.as_secs_f64() - time_measured;
    
    println!("Time Breakdown:");
    println!("├─ Simple prefix: {:.3}s ({:.1}%)", 
        time_simple.as_secs_f64(), (time_simple.as_secs_f64() / elapsed_all.as_secs_f64()) * 100.0);
    println!("├─ Validation:    {:.3}s ({:.1}%)", 
        time_validation.as_secs_f64(), (time_validation.as_secs_f64() / elapsed_all.as_secs_f64()) * 100.0);
    println!("├─ JWT:           {:.3}s ({:.1}%)", 
        time_jwt.as_secs_f64(), (time_jwt.as_secs_f64() / elapsed_all.as_secs_f64()) * 100.0);
    println!("└─ OTHER:         {:.3}s ({:.1}%)\n",
        time_other, (time_other / elapsed_all.as_secs_f64()) * 100.0);
    ...
    println!("├─ Simple:  {:.1}% ({} of {})", 
        ...
        simple_result.count(), all_result.matches.len());
    println!("├─ Validation: {:.1}% ({} of {})", 
        (validation_result.count() as f64 / all_result.matches.len() as f64) * 100.0,
        validation_result.count(), all_result.matches.len());
    println!("├─ JWT:     {:.1}% ({} of {})", 
        (jwt_result.count() as f64 / all_result.matches.len() as f64) * 100.0,
        jwt_result.count(), all_result.matches.len());
    println!("└─ Other:   {:.1}% ({} of {})\n",
        (other_matches as f64 / all_result.matches.len() as f64) * 100.0,
        other_matches, all_result.matches.len());
    ...
    if time_other > elapsed_all.as_secs_f64() * 0.5 {
        ...
        println!("These detectors consume {:.3}s per {:.1}MB = {:.1} MB/s",
            time_other,
