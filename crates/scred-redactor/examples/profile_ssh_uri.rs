...
fn main() {
    ...
    println!(
        "SSH Keys:   {:.1} MB/s ({:.3}s) - {} matches",
        tput_ssh, avg_ssh, ssh_matches
    );
    println!(
        "URI:        {:.1} MB/s ({:.3}s) - {} matches",
        tput_uri, avg_uri, uri_matches
    );
    println!(
        "Combined:   {:.1} MB/s",
        1.0 / (avg_ssh + avg_uri) * (data.len() as f64) / 1_048_576.0
    );
    ...
    println!("\nImprovement Needed:");
    println!(
        "- To reach 300 MB/s: {:.1}x improvement needed",
        300.0 / tput_ssh.min(tput_uri)
    );
    println!(
        "- To reach 400 MB/s: {:.1}x improvement needed",
        400.0 / tput_ssh.min(tput_uri)
    );
}
