...
fn main() {
    ...
    eprintln!(
        "process_chunk:         {:.1} MB/s ({:.0}ms)",
        10.0 / elapsed1.as_secs_f64(),
        elapsed1.as_millis()
    );
    eprintln!(
        "process_chunk_in_place: {:.1} MB/s ({:.0}ms)",
        10.0 / elapsed2.as_secs_f64(),
        elapsed2.as_millis()
    );

    if elapsed2 < elapsed1 {
        let improvement =
            (elapsed1.as_secs_f64() - elapsed2.as_secs_f64()) / elapsed1.as_secs_f64() * 100.0;
