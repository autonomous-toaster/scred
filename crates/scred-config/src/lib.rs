...
"#;
        ...
        let proxy = config.scred_proxy.unwrap();
        assert_eq!(proxy.listen.port, Some(9999));
        assert_eq!(proxy.upstream.url, Some("https://backend.example.com".to_string()));
    }
...
"#;
        ...
        assert_eq!(proxy.rules[0].path, "/api/internal/*");
        assert!(!proxy.rules[0].redact);
    }
    ...
    fn test_defaults() {
        ...
        let cli = config.scred_cli.unwrap();
        assert_eq!(cli.mode, "auto");
        assert!(!cli.streaming);
        assert_eq!(cli.patterns.detect.len(), 3);
        assert_eq!(cli.patterns.redact.len(), 2);
    }
}
...
[scred-proxy.upstream]
url = "https://example.com"

[scred-proxy.redaction]
mode = "selective"

[scred-proxy.redaction.patterns]
detect = ["CRITICAL", "API_KEYS"]
redact = ["CRITICAL"]
"#;
        ...
        assert!(config.scred_proxy.is_some());
        let proxy = config.scred_proxy.unwrap();
        assert_eq!(proxy.listen.port, Some(8888));
        assert_eq!(proxy.listen.address, Some("127.0.0.1".to_string()));
    }

    #[test]
    fn test_config_validation_missing_upstream() {
        let config = FileConfig {
            scred_proxy: Some(ProxyConfig {
    ...
    #[test]
    fn test_config_validation_valid_upstream() {
        let config = FileConfig {
            scred_proxy: Some(ProxyConfig {
                listen: ListenConfig::default(),
    ...
    fn test_environment_override() {
        ...
        let mut file_config = FileConfig {
            scred_proxy: Some(config),
            scred_cli: None,
            scred_mitm: None,
        };

        // Apply environment override
        let result = ConfigLoader::apply_env_overrides(file_config).unwrap();
        
        // Would fail because SCRED_PROXY_REDACTION_MODE is probably not set
        // but the function works as expected
        assert!(result.scred_proxy.is_some());
    }

    #[test]
    fn test_pattern_config_defaults() {
        let patterns = PatternConfig::default();
        assert_eq!(patterns.detect, vec!["CRITICAL", "API_KEYS", "INFRASTRUCTURE"]);
        assert_eq!(patterns.redact, vec!["CRITICAL", "API_KEYS"]);
    }
    ...
    #[test]
    fn test_mitm_ca_cert_defaults() {
        let ca = CaCertConfig::default();
        assert!(ca.generate);
        assert_eq!(ca.path, Some("/tmp/scred-ca.pem".to_string()));
    }

    #[test]
    fn test_redaction_config_defaults() {
        let redaction = RedactionConfig::default();
        assert_eq!(redaction.mode, "selective");
        assert!(redaction.redact_request_body);
        assert!(redaction.redact_request_headers);
        assert!(redaction.redact_response_body);
        assert!(redaction.redact_response_headers);
    }

    #[test]
    fn test_complex_path_rules() {
        let yaml = r#"
scred-proxy:
  listen:
    port: 9999
  ...
  rules:
    - path: "/"
      redact: true
      reason: "Default: redact everything"
...
"#;
        ...
        assert!(config.scred_proxy.is_some());
        let proxy = config.scred_proxy.unwrap();
        assert_eq!(proxy.rules.len(), 4);
        
        // Verify first rule
        assert_eq!(proxy.rules[0].path, "/");
        assert!(proxy.rules[0].redact);
        
        // Verify second rule
        assert_eq!(proxy.rules[1].path, "/health");
        assert!(!proxy.rules[1].redact);
        
        // Verify third rule with wildcard
        assert_eq!(proxy.rules[2].path, "/api/internal/*");
        assert!(!proxy.rules[2].redact);
        
        // Verify fourth rule with custom patterns
        assert!(proxy.rules[3].patterns.is_some());
    }

    #[test]
    fn test_mitm_config_with_upstream_proxy() {
        let yaml = r#"
scred-mitm:
  listen:
    port: 8080
...
"#;
        ...
        let mitm = config.scred_mitm.unwrap();
        
        assert!(mitm.upstream_proxy.is_some());
        let upstream = mitm.upstream_proxy.unwrap();
        assert!(upstream.enabled);
        assert_eq!(upstream.url, Some("http://corporate-proxy:8080".to_string()));
        assert_eq!(upstream.no_proxy.len(), 2);
    }

    #[test]
    fn test_all_sections_together() {
        let yaml = r#"
scred-cli:
  mode: auto
  streaming: true
