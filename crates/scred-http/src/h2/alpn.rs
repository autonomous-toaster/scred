...
mod tests {
    ...
    #[test]
    fn test_from_bytes_h2() {
        assert_eq!(HttpProtocol::from_bytes(b"h2"), Some(HttpProtocol::Http2));
    }

    #[test]
    fn test_from_bytes_http11() {
        assert_eq!(HttpProtocol::from_bytes(b"http/1.1"), Some(HttpProtocol::Http11));
    }
    ...
    fn test_from_bytes_unknown() {
        assert_eq!(HttpProtocol::from_bytes(b"h2c"), None);
        assert_eq!(HttpProtocol::from_bytes(b"unknown"), None);
    }
    ...
    #[test]
    fn test_is_h2() {
        assert!(HttpProtocol::Http2.is_h2());
        assert!(!HttpProtocol::Http11.is_h2());
    }

    #[test]
    fn test_is_http11() {
        assert!(!HttpProtocol::Http2.is_http11());
        assert!(HttpProtocol::Http11.is_http11());
    }

    #[test]
    fn test_display() {
        assert_eq!(format!("{}", HttpProtocol::Http2), "h2 (HTTP/2)");
        assert_eq!(format!("{}", HttpProtocol::Http11), "http/1.1 (HTTP/1.1)");
    }

    #[test]
    fn test_alpn_protocols_list() {
        let protos = alpn_protocols();
        assert_eq!(protos.len(), 2);
        assert!(protos.iter().any(|p| p == b"h2"));
        assert!(protos.iter().any(|p| p == b"http/1.1"));
