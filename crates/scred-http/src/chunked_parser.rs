...
mod tests {
    ...
    fn test_chunked_parser_creation() {
        ...
        assert_eq!(parser.state, ChunkState::ReadingSize);
        assert!(!parser.is_complete());
    }

    #[test]
    fn test_hex_chunk_size_parsing() {
        // Test that we can parse hex sizes
        let sizes = vec![
            ("10", 16),      // decimal 16
            ("ff", 255),     // decimal 255
            ("1f", 31),      // decimal 31
        ...
        for (hex_str, expected) in sizes {
            let parsed = usize::from_str_radix(hex_str, 16).unwrap();
            assert_eq!(parsed, expected);
        }
    }

    #[test]
    fn test_lookahead_buffer_maintenance() {
        let mut parser = ChunkedParser::new();

        // First chunk
        let chunk1 = b"Hello World";
        parser.maintain_lookahead(chunk1);
        assert!(!parser.lookahead_buffer.is_empty());

        // Second chunk smaller than lookahead
        let chunk2 = b"123";
        parser.maintain_lookahead(chunk2);
        assert!(parser.lookahead_buffer.len() <= parser.lookahead_max);
    }
    ...
    fn test_chunk_state_transitions() {
        ...
        // Simulate size parsing
        parser.current_chunk_size = 100;
        parser.state = ChunkState::ReadingData;
        assert_eq!(parser.state, ChunkState::ReadingData);

        // Simulate reading 0-sized chunk (final)
        parser.current_chunk_size = 0;
        parser.state = ChunkState::ReadingTrailers;
        assert_eq!(parser.state, ChunkState::ReadingTrailers);
