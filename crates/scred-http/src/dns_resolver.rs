...
mod tests {
    ...
    fn test_strip_scheme_http() {
        assert_eq!(
            DnsResolver::strip_scheme("http://proxy.example.com:3128"),
    ...
    fn test_strip_scheme_https() {
        assert_eq!(
            DnsResolver::strip_scheme("https://proxy.example.com:3128"),
            "proxy.example.com:3128"
        );
    }
    ...
    fn test_strip_scheme_no_scheme() {
        assert_eq!(
            DnsResolver::strip_scheme("proxy.example.com:3128"),
            "proxy.example.com:3128"
        );
    }

    #[test]
    fn test_strip_scheme_ip_address() {
        assert_eq!(
            DnsResolver::strip_scheme("http://192.168.1.1:8080"),
            "192.168.1.1:8080"
        );
    }
    ...
    async fn test_connect_localhost() {
        // This should succeed (localhost is always available)
        // Note: requires a server listening on 127.0.0.1:9999 for this to work
        // Skipping in unit tests as it requires external setup
        let result = DnsResolver::connect_with_retry("127.0.0.1:22").await;
        // Either succeeds or fails with connection refused (both acceptable)
        // The important thing is DNS resolution didn't fail
    ...
    async fn test_invalid_address_format() {
        ...
        let result = DnsResolver::connect_with_retry(":::invalid:::").await;
        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_unreachable_address() {
        // This should fail after retries (no server on this port)
        let result = DnsResolver::connect_with_retry("127.0.0.1:1").await;
        // Should fail, but not with DNS error
        let _ = result;
    }

    #[tokio::test]
    async fn test_strip_scheme_with_url_in_connect() {
        // Test that connect_with_retry handles URLs with schemes
        let result = DnsResolver::connect_with_retry("http://127.0.0.1:1").await;
        // Should fail (port 1 not accessible), not with DNS error
        let _ = result;
    }
}
