...
mod tests {
    ...
    #[test]
    fn test_secret_keyword_detection() {
        let secret_env = "API_KEY=sk-abc123\nTOKEN=xyz789\nPASSWORD=secret123\n";
        let result = detect_format(secret_env.as_bytes());
        assert_eq!(result.mode, DetectionMode::EnvFormat);
    ...
    fn test_aws_config_detection() {
        let aws_config = "aws_access_key_id=AKIA123\naws_secret_access_key=wJalrXUtn\n";
        let result = detect_format(aws_config.as_bytes());
        assert_eq!(result.mode, DetectionMode::EnvFormat);
    }

    #[test]
    fn test_text_format_detection() {
        let log_file = "2024-03-19 10:30:45 INFO Starting application\n\
                        2024-03-19 10:30:46 DEBUG Loading config\n\
                        2024-03-19 10:30:47 ERROR Connection failed\n";
        let result = detect_format(log_file.as_bytes());
        assert_eq!(result.mode, DetectionMode::TextFormat);
    }

    #[test]
    fn test_binary_detection_null_bytes() {
        let binary = b"some text\x00more text";
        let result = detect_format(binary);
        assert_eq!(result.mode, DetectionMode::BinaryDetected);
    }

    #[test]
    fn test_mixed_content_defaults_to_text() {
        let mixed = "DEBUG=true\nSome log message with text\nHOSTNAME=localhost\n";
        let result = detect_format(mixed.as_bytes());
        // Mixed content with low env-var density should default to text
        assert_eq!(result.mode, DetectionMode::TextFormat);
    }
    ...
    #[test]
    fn test_single_line() {
        let result = detect_format(b"API_KEY=secret123");
        // Single line should still be detected if it matches
        assert_eq!(result.mode, DetectionMode::EnvFormat);
    }

    #[test]
    fn test_comments_ignored() {
        let with_comments = "# Configuration file\nAPI_KEY=secret123\n# End of config\n";
        let result = detect_format(with_comments.as_bytes());
        assert_eq!(result.mode, DetectionMode::EnvFormat);
    }
}
