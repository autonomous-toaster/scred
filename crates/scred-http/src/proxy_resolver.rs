...
mod tests {
    ...
    #[test]
    fn test_parse_no_proxy_simple() {
        let entries = parse_no_proxy_list("localhost,127.0.0.1");
        assert_eq!(entries.len(), 2);
    }

    #[test]
    fn test_parse_no_proxy_domain_suffix() {
        let entries = parse_no_proxy_list(".example.com,.internal.com");
        assert_eq!(entries.len(), 2);
    }

    #[test]
    fn test_parse_no_proxy_with_spaces() {
        let entries = parse_no_proxy_list("localhost, .example.com, 192.168.0.0/16");
        assert_eq!(entries.len(), 3);
    }

    #[test]
    fn test_should_bypass_localhost() {
        let resolver = MitmConfig {
            http_proxy: Some("http://proxy.example.com:8080".to_string()),
            https_proxy: None,
            no_proxy_list: vec![NoProxyEntry::Localhost],
        };

        assert!(resolver.should_bypass_proxy("localhost"));
        assert!(resolver.should_bypass_proxy("127.0.0.1"));
    }
    ...
    fn test_should_bypass_domain_suffix() {
        let resolver = MitmConfig {
            http_proxy: Some("http://proxy.example.com:8080".to_string()),
            https_proxy: None,
            no_proxy_list: vec![NoProxyEntry::Suffix(".internal.com".to_string())],
        };

        assert!(resolver.should_bypass_proxy("api.internal.com"));
        assert!(resolver.should_bypass_proxy("db.internal.com"));
        assert!(!resolver.should_bypass_proxy("example.com"));
    }

    #[test]
    fn test_get_proxy_for_https() {
        let resolver = MitmConfig {
            http_proxy: Some("http://proxy1:8080".to_string()),
            https_proxy: Some("http://proxy2:8080".to_string()),
            no_proxy_list: vec![],
        };

        assert_eq!(
            resolver.get_proxy_for("example.com", true),
            Some("http://proxy2:8080".to_string())
        );
    }

    #[test]
    fn test_get_proxy_for_http() {
        let resolver = MitmConfig {
            http_proxy: Some("http://proxy1:8080".to_string()),
            https_proxy: None,
            no_proxy_list: vec![],
        ...
        assert_eq!(
            resolver.get_proxy_for("example.com", false),
            Some("http://proxy1:8080".to_string())
        );
    }

    #[test]
    fn test_get_proxy_bypassed() {
        let resolver = MitmConfig {
            http_proxy: Some("http://proxy:8080".to_string()),
            https_proxy: None,
            no_proxy_list: vec![NoProxyEntry::Host("example.com".to_string())],
        };

        assert_eq!(resolver.get_proxy_for("example.com", false), None);
    }
}
