...
fn main() {
    ...
    for i in 0..iterations {
        ...
        if i == 0 {
            ...
            println!("First engine creation: {:.2} ms", first_elapsed.as_secs_f64() * 1000.0);
        }
    }
    let total_elapsed = start.elapsed();
    
    let avg_ms = total_elapsed.as_secs_f64() * 1000.0 / iterations as f64;
    println!("Average engine creation ({} iterations): {:.2} ms", iterations, avg_ms);
    println!("Throughput: {:.1} engines/sec", iterations as f64 / total_elapsed.as_secs_f64());
}
