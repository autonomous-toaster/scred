...
### 6. STREAMING RESPONSE HANDLER - PATH UNCLEAR

**File**: `crates/scred-http/src/streaming_response.rs`

Need to check:
- Does it use StreamingRedactor or RedactionEngine?
- Are selectors passed to it?
- What parameters does it accept?

**Suspicion**: Likely uses StreamingRedactor without selectors

---

### 7. MITM TLS HANDLER - COMPLETELY UNKNOWN

**File**: `crates/scred-mitm/src/mitm/tls_mitm.rs` and related

Critical unknowns:
- What redaction engine does HTTPS MITM use?
- Are selectors passed to it?
- Does HTTPS redaction use same logic as HTTP?
- Are H2 frames redacted with selectors?

**Suspicion**: Likely uses RedactionEngine without selectors

---

### 8. DETECT VS REDACT SELECTOR ASYMMETRY

Current behavior:
- `--detect` and `--redact` are separate selectors
- CLI: Both respected ✅
- Proxy: Neither respected ❌
- MITM: Neither respected ❌

**Issue**: If user wants `--detect ALL --redact CRITICAL`:
- CLI: Detects all patterns, redacts only CRITICAL ✅
- Proxy/MITM: Probably redacts all patterns ❌

**Test needed**: Does asymmetry work in CLI but not Proxy/MITM?

---

### 9. MISSING SELECTOR PROPAGATION CHAIN

CLI → Proxy → Upstream:
```
CLI (local redaction)
  ↓ [selectors work here ✅]
Proxy (forwarding redaction)
  ↓ [selectors don't work here ❌]
Upstream (receives data)
```

When Proxy sits between CLI and upstream:
- If CLI doesn't redact, Proxy gets unredacted data
- Proxy should redact with selectors
- But Proxy doesn't enforce selectors
- Result: Secret gets through

---

### 10. PATTERN FILTER EFFECTIVENESS

Unknown: When a selector says "CRITICAL only", does it mean:
- **Option A**: Use ONLY those patterns, ignore others
- **Option B**: Ensure those patterns are included, may add others
- **Option C**: Something else entirely

Current code behavior:
- CLI: Option A (only selected patterns) ✅
- Proxy: Option ? (unknown, probably uses all) ❌
- MITM: Option ? (unknown, probably uses all) ❌

If interpretation differs, same config means different things in different tools.


---

## 🔴 CRITICAL FINDING #11: MITM H2 HANDLER IGNORES SELECTORS
...
async fn handle_stream(
    ...
    ...
    detect_patterns: scred_http::PatternSelector,  // ← PASSED
    redact_patterns: scred_http::PatternSelector,   // ← PASSED
) -> Result<()> {
    ...
    let redacted = engine.redact(&body_str);  // ← NOT USED!
    // Uses RedactionEngine.redact(), ignores detect_patterns and redact_patterns
}
```

**Issue**: Selectors are passed to the function but NEVER USED
- Selectors extracted in tls_mitm.rs (line 141-142)
- Selectors passed to h2_mitm_handler (line 181-182)
- Selectors completely IGNORED in handle_stream (line 143)
- Uses engine.redact() which applies ALL patterns

**Evidence of Dead Code**:
- Selectors passed but not stored in self
- Selectors used only for passing to next level
- Never dereferenced or used for filtering
- Could be deleted and code would work the same

**Impact**: HTTPS MITM (HTTP/2) IGNORES SELECTORS
- User configures `--redact CRITICAL`
- MITM HTTPS connections use ALL patterns
- Selectors silently ignored
...
1. **HTTP Handler** (Proxy + MITM) - Uses RedactionEngine, ignores selectors
2. **Proxy Streaming** - Creates unused ConfigurableEngine, uses StreamingRedactor
3. **MITM HTTP** - Doesn't pass selectors to http_handler
4. **MITM H2** - Accepts selectors but doesn't use them

### All Major Code Paths Broken
- ✅ CLI: WORKS (uses ConfigurableEngine)
- ❌ Proxy HTTP: BROKEN (uses RedactionEngine)
- ❌ Proxy Streaming: BROKEN (uses StreamingRedactor)
- ❌ MITM HTTP: BROKEN (uses RedactionEngine)
- ❌ MITM HTTPS (H2): BROKEN (uses RedactionEngine, ignores selectors)

**Conclusion**: Selectors work ONLY in CLI. Production tools (Proxy, MITM) ignore them completely.
